...
public class RobotTreasureHunt implements KeyEventDispatcher {
	...
	private void spaceBarWasPressed() {
		...
		System.out.println("x : " + Math.abs(robotXLocation - 730) + "\t  Y : " + Math.abs(robotYLocation - 430));
		if (Math.abs(robotXLocation - 730) < 20 && Math.abs(robotYLocation - 430) < 20) {
		...
		if (Math.abs(robotXLocation - 520) < 20 && Math.abs(robotYLocation - 370) < 20) {
			JOptionPane.showMessageDialog(null, "almost there");
			JOptionPane.showMessageDialog(null, "go to the robot he has the final clue");
